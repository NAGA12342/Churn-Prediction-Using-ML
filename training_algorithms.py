...
def knn_algorithm(X_train,X_test,y_train,y_test):
    try:
        ...
        logger.info(f'Confusion matrics KNN : {confusion_matrix(y_test, knn_reg.predict(X_test))}')
...
def Decision_tree_algorithm(X_train,X_test,y_train,y_test):
    try:
        ...
        logger.info(f'Confusion matrics DT : {confusion_matrix(y_test, DT_reg.predict(X_test))}')
...
def Random_forest_algorithm(X_train,X_test,y_train,y_test):
    try:
        ...
        logger.info(f'Confusion matrics RF : {confusion_matrix(y_test, RF_reg.predict(X_test))}')
...
def Gradient_Boosting_algorithm(X_train,X_test,y_train,y_test):
    try:
        GB_reg = GradientBoostingClassifier()
        GB_reg.fit(X_train, y_train)
        logger.info(f'test Accuracy GB : {accuracy_score(y_test, GB_reg.predict(X_test))}')
        logger.info(f'Confusion matrics GB : {confusion_matrix(y_test, GB_reg.predict(X_test))}')
...
def SVM_algorithm(X_train,X_test,y_train,y_test):
    try:
        ...
        logger.info(f'Confusion matrics SVM : {confusion_matrix(y_test, SVM_reg.predict(X_test))}')
...
def Navie_bayes_algorithm(X_train,X_test,y_train,y_test):
    try:
        NB_reg = GaussianNB()
        NB_reg.fit(X_train, y_train)
        logger.info(f'test Accuracy NB : {accuracy_score(y_test, NB_reg.predict(X_test))}')
        logger.info(f'Confusion matrics NB : {confusion_matrix(y_test, NB_reg.predict(X_test))}')
...
def Logistic_Regression_algorithm(X_train,X_test,y_train,y_test):
    try:
        ...
        logger.info(f'Confusion matrics LR : {confusion_matrix(y_test, lg_reg.predict(X_test))}')
...
def XGB_algorithm(X_train,X_test,y_train,y_test):
    try:
        XGB_reg = XGBClassifier()
        XGB_reg.fit(X_train, y_train)
        logger.info(f'test Accuracy XGB : {accuracy_score(y_test, XGB_reg.predict(X_test))}')
        logger.info(f'Confusion matrics XGB : {confusion_matrix(y_test, XGB_reg.predict(X_test))}')
        logger.info(f'Classification report XGB : {classification_report(y_test, XGB_reg.predict(X_test))}')
        global XGB_pred
        ...
        er_ty, er_msg, er_lin = sys.exc_info()
        logger.info(f'Issue is : {er_lin.tb_lineno} : due to {er_msg}')
def best_model_using_auc_roc(X_train,X_test,y_train,y_test):
    try:
...
def common(X_train,X_test,y_train,y_test):
    try:
        ...
        best_model_using_auc_roc(X_train,X_test,y_train,y_test)
