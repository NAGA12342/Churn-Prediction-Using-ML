...
class CHURN_PROJECT_INFO:
    ...
    def missing_values(self):
        try:
            ...
            # Drop customerID column from both train and test sets (if present)
    ...
    def handle_outliers(self):
        try:
            #lets convert each numerical collumn into log transformation
            #70% of outliers are can remove
            #then we pass to trimming formula to all outliers in the data
    ...
    def feature_selection(self):
        try:
            ...
            logger.info(f'Columns names : {self.X_train_num.columns}')
            logger.info(f'Columns names : {self.X_test_num.columns}')
    ...
    def cat_to_num(self):
        try:
            ...
            oh.fit(
                self.X_train_cat[['gender', 'Partner', 'Dependents', 'PhoneService', 'MultipleLines', 'InternetService',
                                  'OnlineSecurity', 'OnlineBackup', 'DeviceProtection', 'TechSupport', 'StreamingTV',
                                  'StreamingMovies', 'PaperlessBilling', 'PaymentMethod']])
            logger.info(f'{oh.categories_}')
            logger.info(f'{oh.get_feature_names_out()}')

            res_oh = oh.transform(self.X_train_cat[['gender', 'Partner', 'Dependents', 'PhoneService', 'MultipleLines',
                                                    'InternetService', 'OnlineSecurity', 'OnlineBackup',
                                                    'DeviceProtection',
                                                    'TechSupport', 'StreamingTV', 'StreamingMovies', 'PaperlessBilling',
                                                    'PaymentMethod']]).toarray()
            res_test = oh.transform(self.X_test_cat[['gender', 'Partner', 'Dependents', 'PhoneService', 'MultipleLines',
                                                     'InternetService', 'OnlineSecurity', 'OnlineBackup',
                                                     'DeviceProtection',
            ...
            self.X_train_cat = self.X_train_cat.drop(['gender', 'Partner', 'Dependents', 'PhoneService', 'MultipleLines',
                 'InternetService', 'OnlineSecurity', 'OnlineBackup', 'DeviceProtection',
                 'TechSupport', 'StreamingTV', 'StreamingMovies', 'PaperlessBilling',
                 'PaymentMethod', 'Contract'], axis=1)
            self.X_test_cat = self.X_test_cat.drop(['gender', 'Partner', 'Dependents', 'PhoneService', 'MultipleLines',
                                                    'InternetService', 'OnlineSecurity', 'OnlineBackup',
                                                    'DeviceProtection',
                                                    'TechSupport', 'StreamingTV', 'StreamingMovies', 'PaperlessBilling',
                                                    'PaymentMethod', 'Contract'], axis=1)
            ...
            with open('C:\\Users\\tnaga\\OneDrive\\Documents\\Churn_Project\\Result\\feature_columns.pkl', 'wb') as f:
            ...
            #logger.info(f'{self.y_train_unique()}')
            ...
            #Dependent variable should beconverted using label encoder
    ...
    def combining_both_cat_num(self):
        try:
            ...
            logger.info(
                f'training data shape : {self.training_data.shape} : training data columns : {self.training_data.columns}')
            logger.info(
                f'testing data shape : {self.testing_data.shape} : testing data columns : {self.testing_data.columns}')
    ...
    def balanced_data(self):
        try:
            ...
            logger.info(f'total rows for good categories training data {self.training_data.shape[0]} was : {sum(self.y_train == 1)}')
            logger.info(f'total rows for good categories training data {self.training_data.shape[0]} was : {sum(self.y_train == 0)}')
            ...
            logger.info(f'total rows for good categories training data {self.training_data_res.shape[0]} was : {sum(self.y_train_res == 1)}')
            logger.info(f'total rows for good categories training data {self.training_data_res.shape[0]} was : {sum(self.y_train_res == 0)}')
    ...
    def feature_scaling(self):
        try:
            ...
            self.testing_data_transformstion = sc.transform(self.testing_data)
            with open('standard_scalar.pkl', 'wb') as t:
                pickle.dump(sc, t)
            logger.info(f'....................After Feature Scaling.............................')
    ...
    #     try:
    #         common(self.training_data_res_transformation,self.y_train_res,self.testing_data_transformstion,self.y_test)
    #
    #     except Exception as e:
    #         er_ty, er_msg, er_lin = sys.exc_info()
    #         logger.info(f'Issue is : {er_lin.tb_lineno} : due to {er_msg}')
    def best_model(self):
        try:
            ...
            logger.info(f'Model Test Accuracy : {accuracy_score(self.y_test, self.reg_log.predict(self.testing_data_transformstion))}')
            logger.info(f'Confusion Matrix : {confusion_matrix(self.y_test, self.reg_log.predict(self.testing_data_transformstion))}')
            logger.info(f'Classification Report : {classification_report(self.y_test, self.reg_log.predict(self.testing_data_transformstion))}')

            logger.info(f'.............Model Saving..............')
            with open('churn_prediction_project.pkl','wb') as f:
...
if __name__ == "__main__":
    try:
        path = 'C:\\Users\\tnaga\\OneDrive\\Documents\\Churn_Project\\WA_Fn-UseC_-Telco-Customer-Churn.csv'
